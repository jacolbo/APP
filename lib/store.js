...
function withFolderDefaults(folder) {
  return {
    ...
    watermarkText: null,
    slug: null,
    // Per-gallery switches, mirroring the On/Off rows a studio expects.
    // Downloads still need a PIN on top of this; the switch only decides
    // whether the option is offered at all.
    downloadsEnabled: true,
    favouritesEnabled: true,
    slideshowEnabled: true,
    sharingEnabled: true,
    // Delivering finished work from Drive. `releaseTrigger` stays 'always' for
    // every existing gallery, so nothing that worked before starts asking to
    // be released.
    driveFolderId: null,
    driveFolderName: '',
    releaseTrigger: 'always',   // always | manual | drive-public
    releasedAt: null,
    deliveryEmail: '',
...
function withSelectionDefaults(selection) {
  return { clientEmail: '', selected: true, rating: 0, ...selection };
}
...
export class Store {
  ...
  folderByLink(uniqueLink) {
    ...
    const wanted = String(uniqueLink);
    return this.data.folders.find((f) => f.uniqueLink === wanted)
      || this.data.folders.find((f) => f.slug && f.slug === wanted.toLowerCase())
      || null;
  }

  /** Slugs share a namespace with generated links, so neither can shadow the other. */
  slugTaken(slug, exceptFolderId) {
    const wanted = String(slug).toLowerCase();
    return this.data.folders.some((f) => f.id !== exceptFolderId
      && (f.slug === wanted || f.uniqueLink === wanted));
  }

  /**
   * Copies a folder's structure — its tabs and their settings — without the
   * images or anyone's picks. Duplicating a shoot means reusing how it was set
   * up, not shipping last client's photos to the next one.
   */
  duplicateFolder(id, title) {
    const source = this.folder(id);
    if (!source) return null;
    const copy = this.createFolder({
      parentId: source.parentId,
      title,
      clientName: source.clientName,
      description: source.description,
      withDefaultTabs: false,
    });
    Object.assign(copy, {
      brandColor: source.brandColor,
      watermarkText: source.watermarkText,
      webhookUrl: source.webhookUrl,
      webhookEvents: source.webhookEvents,
      downloadsEnabled: source.downloadsEnabled,
      favouritesEnabled: source.favouritesEnabled,
      slideshowEnabled: source.slideshowEnabled,
      sharingEnabled: source.sharingEnabled,
      // How this studio releases work carries over; the fact that the last
      // shoot was already delivered must not. A duplicate is a new job.
      releaseTrigger: source.releaseTrigger,
    });
    for (const tab of this.tabsIn(source.id)) {
      this.createTab({
        folderId: copy.id,
        title: tab.title,
        access: tab.access,
        downloadable: tab.downloadable,
        defer: true,
      });
    }
    this.save();
  ...
  /** True once an expiry date has passed. Draft/published is a separate axis. */
  hasExpired(folder) {
    return Boolean(folder && folder.expiresAt && Date.parse(folder.expiresAt) <= Date.now());
  }

  /**
   * Folders whose expiry has just passed and which have not been marked yet.
   * Marking is what stops the same gallery being reported over and over.
   */
  claimNewlyExpired() {
    const claimed = this.data.folders.filter((f) => this.hasExpired(f) && !f.archivedAt);
    if (!claimed.length) return [];
    for (const folder of claimed) folder.archivedAt = nowIso();
    this.save();
    return claimed;
  }
  ...
  countView(folder) {
    ...
    folder.lastViewedAt = nowIso();
    this.save();
  }

  countDownload(folder, amount = 1) {
    folder.downloads = (folder.downloads || 0) + amount;
    this.save();
  }
  ...
  /** Favourites only. A row kept for its note or rating is not a favourite. */
  selectionsBy(rootFolderId, clientSessionId) {
    return this.data.selections.filter(
      (s) => s.rootFolderId === rootFolderId && s.clientSessionId === clientSessionId && s.selected !== false,
    );
  }

  /** Every row, favourited or not — what the studio reads for notes and stars. */
  feedbackBy(rootFolderId, clientSessionId) {
    return this.data.selections.filter(
  ...
  feedbackOn(imageId) {
    return this.data.selections.filter((s) => s.imageId === imageId);
  }

  /** Upsert or remove one favourite. (imageId, clientSessionId) is unique. */
  setSelection({ rootFolderId, imageId, clientSessionId, clientName, clientEmail, note, rating, selected }) {
    ...
    if (selected === false) {
      ...
      // Un-hearting a photo should not silently throw away the comment or the
      // stars the client left on it; the row survives without the heart.
      if (existing.note || existing.rating) {
    ...
    if (existing) {
      ...
      if (typeof note === 'string') existing.note = note;
      if (typeof rating === 'number') existing.rating = rating;
      if (selected === true) existing.selected = true;
      existing.updatedAt = nowIso();
      this.save();
      return existing.selected === false ? null : existing;
    }
    const selection = {
      id: newId('sel'),
      rootFolderId,
      ...
      // Only an explicit heart makes something a favourite. Leaving a star or
      // a note must not quietly add the photo to the client's picks.
  ...
  identify(rootFolderId, clientSessionId, { clientName, clientEmail }) {
    let touched = 0;
    for (const selection of this.feedbackBy(rootFolderId, clientSessionId)) {
