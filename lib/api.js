...
import { driveFromEnv, folderUrl, parseFolderId } from './drive.js';
...
// A single Drive original is buffered to compute its zip CRC, so it needs a
// ceiling of its own — independent of the upload limit, which governs what the
// browser is allowed to push at us.
const MAX_DRIVE_FILE_BYTES = 200 * 1024 * 1024;
// Drive ids are opaque but well-formed; anything else never reaches Google.
...
export function createApi({ store, config }) {
  ...
  // Built once. Null is a normal, supported state: Drive is optional and the
  // app is fully usable without it.
  const drive = driveFromEnv();

  const requireDrive = () => {
    if (!drive) {
      throw httpError(503, 'Google Drive is not set up on this server. Add the service account key and restart.');
    }
    return drive;
  };

  /**
   * Whether the studio has released this gallery's downloads.
   *
   * Three triggers, because a studio that already used this app should not
   * suddenly find its galleries locked:
   *   always       what every existing gallery does — no release step
   *   manual       a switch in the app; nothing in Drive is made public
   *   drive-public downloads follow the Drive folder's sharing state
   *
   * The Drive check is cached briefly: a client clicking around a gallery must
   * not turn into a Drive API call per click.
   */
  const releaseCache = new Map();
  const RELEASE_CACHE_MS = 60 * 1000;

  async function releaseState(folder) {
    const trigger = folder.releaseTrigger || 'always';
    if (trigger === 'always') return { released: true, trigger, reason: '' };
    if (trigger === 'manual') {
      return {
        released: Boolean(folder.releasedAt),
        trigger,
        releasedAt: folder.releasedAt || null,
        reason: folder.releasedAt ? '' : 'Not released yet.',
      };
    }

    // drive-public
    if (!folder.driveFolderId) {
      return { released: false, trigger, reason: 'No Drive folder is attached to this gallery.' };
    }
    const cached = releaseCache.get(folder.id);
    if (cached && Date.now() < cached.until) return cached.value;

    let value;
    try {
      const state = await requireDrive().publicState(folder.driveFolderId);
      value = state.isPublic === true
        ? { released: true, trigger, reason: '' }
        : {
          released: false,
          trigger,
          // null means Google would not tell us, which is not the same as
          // "private" — say which one it was.
          unknown: state.isPublic === null,
          reason: state.isPublic === null
            ? (state.reason || 'Could not read the folder\'s sharing settings.')
            : 'The Drive folder is not shared publicly yet.',
        };
    } catch (err) {
      // Fail closed. A Drive outage must not hand out finished work.
      value = { released: false, trigger, unknown: true, reason: err.message };
    }
    releaseCache.set(folder.id, { until: Date.now() + RELEASE_CACHE_MS, value });
    return value;
  }

  const forgetRelease = (folderId) => releaseCache.delete(folderId);

  async function requireReleased(folder) {
    const release = await releaseState(folder);
    if (release.released) return;
    throw httpError(403, release.unknown
      ? 'Downloads are unavailable right now. Please try again shortly.'
      : 'These photos have not been released yet.');
  }

  const driveIdParam = (value) => {
    const id = text(value, 200);
    if (!id) return null;
    if (!DRIVE_ID.test(id)) throw httpError(400, 'That is not a valid Drive file id');
    return id;
  };
  ...
  const adminImage = (image) => ({
    ...
    // Whether the original still lives in Drive, so the studio can see at a
    // glance which photos depend on it. Deliberately a flag and not the Drive
    // id: nothing outside this server has any business addressing the file.
    source: image.driveFileId ? 'drive' : 'upload',
  });
  ...
  route('GET', '/api/folders/:id', ({ res, params }) => {
    ...
    sendJson(res, 200, {
      ...
      selections: selections.map((s) => ({
        id: s.id,
        imageId: s.imageId,
        // The studio copies these into Lightroom, so the name travels with
        // the pick rather than having to be looked up tab by tab — picks in
        // sub-folders are not in the tabs this payload carries.
        fileName: (store.image(s.imageId) || {}).originalName || '',
  ...
  route('PATCH', '/api/folders/:id', async ({ req, res, params }) => {
    ...
    if ('releaseTrigger' in body) {
      const trigger = text(body.releaseTrigger, 20);
      if (!['always', 'manual', 'drive-public'].includes(trigger)) {
        throw httpError(400, 'releaseTrigger must be always, manual or drive-public');
      }
      folder.releaseTrigger = trigger;
      forgetRelease(folder.id);
    }
    if ('releasedAt' in body) {
      // Truthy releases now; explicitly false pulls it back.
      folder.releasedAt = body.releasedAt ? nowIso() : null;
      forgetRelease(folder.id);
    }
    if ('driveFolderId' in body) {
      folder.driveFolderId = driveIdParam(body.driveFolderId);
      folder.driveFolderName = text(body.driveFolderName, 160);
      forgetRelease(folder.id);
    }
    if ('deliveryEmail' in body) folder.deliveryEmail = text(body.deliveryEmail, 160);
  ...
  // Lets the admin page show the right thing without guessing: a setup hint
  // when Drive is unconfigured, the import button when it is.
  route('GET', '/api/drive/status', ({ res }) => {
    sendJson(res, 200, {
      configured: Boolean(drive),
      clientEmail: drive ? drive.clientEmail : '',
    });
  });

  // Lists a pasted Drive folder so the studio can see what is in it before
  // importing anything.
  route('POST', '/api/drive/list', async ({ req, res }) => {
    const client = requireDrive();
    const body = await readJson(req);
    const folderId = parseFolderId(body.folder);
    if (!folderId) throw httpError(400, 'Paste a Google Drive folder link, or its id');

    const listing = await client.listFolder(folderId);
    sendJson(res, 200, { folderId, ...listing });
  });

  /**
   * Streams one Drive original to the signed-in studio so the browser can
   * scale it down. This is the only path by which Drive bytes leave the
   * server, and it is admin-only — a client never reaches it.
   *
   * It exists because the server has no image library: resizing happens in the
   * admin's canvas, exactly as it does for a dragged-in file.
   */
  route('GET', '/api/drive/files/:fileId/content', async ({ res, params }) => {
    const client = requireDrive();
    const fileId = driveIdParam(params.fileId);
    if (!fileId) throw httpError(400, 'Missing Drive file id');

    const upstream = await client.fileContent(fileId);
    const type = upstream.headers.get('content-type') || 'application/octet-stream';
    if (!IMAGE_TYPES.has(type.split(';')[0].trim())) {
      throw httpError(415, 'That Drive file is not an image this gallery can use.');
    }
    ...
    // Pipe rather than buffer: a 60 MB original should not sit in memory.
  ...
  route('POST', '/api/tabs/:id/images', async ({ req, res, params, url }) => {
    ...
    const image = store.addImage({
      ...
      // Set when the preview was made from a Drive original that stays in
      // Drive. Null means the bytes on disk are the deliverable.
      driveFileId: driveIdParam(url.searchParams.get('driveId')),
      title: '',
      notes: '',
      tags: [],
  ...
  async function serveImage(req, res, kind, imageId) {
    ...
    if (!admin) {
      ...
      if (kind === 'd') {
        ...
        // Finished work stays locked until the studio releases it.
        if (top) {
          const release = await releaseState(top);
          if (!release.released) return sendError(res, 403, 'These photos have not been released yet');
        }
      }
    }
  ...
  async function streamZip(res, images, archiveName) {
    ...
    for (const image of images) {
      ...
      if (image.driveFileId) {
        // The deliverable never touched this disk. Fetch it now — the caller
        // has already proved the PIN, so this is the one moment a client is
        // entitled to the full-resolution file.
        const drive = requireDrive();
        const body = await drive.fileBuffer(image.driveFileId, MAX_DRIVE_FILE_BYTES);
        await zip.addBuffer(body, name, when);
      } else {
        await zip.add(store.filePath(image.file), name, when);
      }
    }
    await zip.finish();
  }
  ...
  publicRoute('GET', '/api/g/:link/selection.zip', async ({ req, res, url, params }) => {
    ...
    await requireReleased(rootFolder);
  ...
  route('GET', '/api/folders/:id/picks/:sessionId/zip', async ({ res, params }) => {
    const folder = requireFolder(params.id);
    const images = store
      .selectionsBy(folder.id, text(params.sessionId, 64))
      .map((selection) => store.image(selection.imageId))
      .filter(Boolean);
    if (!images.length) throw httpError(404, 'That list is empty');
    const owner = store.feedbackBy(folder.id, params.sessionId)[0] || {};
    await streamZip(res, images, `${folder.title} - ${owner.clientName || owner.clientEmail || 'picks'}`);
  });

  /**
   * The shortlist as a spreadsheet, which is what most studios actually want
   * to keep: one row per picked photo, with who picked it and what they said.
   */
  route('GET', '/api/folders/:id/picks.csv', ({ res, params }) => {
    const folder = requireFolder(params.id);
    const cell = (value) => `"${String(value === undefined || value === null ? '' : value).replace(/"/g, '""')}"`;
    const rows = [['Client', 'Email', 'File', 'Folder', 'Tab', 'Rating', 'Note', 'Picked at']];

    for (const selection of store.selectionsIn(folder.id)) {
      ...
      rows.push([
        ...
        store.pathOf(image.folderId).map((f) => f.title).join(' / '),
        tab ? tab.title : '',
        selection.rating || '',
        selection.note,
        selection.createdAt,
    ...
    const name = String(folder.title).replace(/[^\w.\- ]/g, '_');
    res.writeHead(200, {
      // A BOM so Excel opens it as UTF-8 rather than mangling accented names.
      'content-type': 'text/csv; charset=utf-8',
      'content-disposition': `attachment; filename="${name} picks.csv"`,
      'cache-control': 'no-store',
    });
    res.end(`\uFEFF${csv}`);
  });
